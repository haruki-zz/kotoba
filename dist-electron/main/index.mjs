...
const defaultSm2State = (now = Date.now()) => ({
  repetition: 0,
  interval: 0,
...
const isDueForReview = (sm2, now = Date.now()) => sm2.next_review_at <= now;
const sortByNextReview = (words) => [...words].sort((a, b) => {
  if (a.sm2.next_review_at === b.sm2.next_review_at) {
    return a.updated_at - b.updated_at;
  }
  return a.sm2.next_review_at - b.sm2.next_review_at;
});
const buildReviewQueue = (words, now = Date.now()) => {
  const dueWords = words.filter((word) => isDueForReview(word.sm2, now));
  return sortByNextReview(dueWords);
};
const applyReviewResult = (word, grade, now = Date.now()) => {
...
const requireWordDraft = (draft) => {
  if (typeof draft !== "object" || draft === null) {
    throw new Error("词条数据需为对象");
  }
  return draft;
};
const requireWordUpdate = (update) => {
...
const createIpcHandlers = ({ dataStore: dataStore2, providerManager: providerManager2, now }) => {
  ...
  return {
    [IPC_CHANNELS.getProvider]: async () => providerManager2.getState(),
    [IPC_CHANNELS.setProvider]: async (payload) => providerManager2.setConfig(payload ?? {}),
    [IPC_CHANNELS.generateWordCard]: async (payload) => {
      const provider = providerManager2.getProvider();
      return provider.generateWordCard(requireTerm(payload?.term));
    },
    [IPC_CHANNELS.loadWords]: async () => dataStore2.loadWords(getNow()),
    [IPC_CHANNELS.addWord]: async (draft) => dataStore2.addWord(requireWordDraft(draft), getNow()),
    [IPC_CHANNELS.updateWord]: async (payload) => dataStore2.updateWord(requireId(payload?.id), requireWordUpdate(payload?.update ?? {}), getNow()),
    [IPC_CHANNELS.deleteWord]: async (payload) => dataStore2.deleteWord(requireId(payload?.id), getNow()),
    [IPC_CHANNELS.loadReviewQueue]: async () => {
      const words = await dataStore2.loadWords(getNow());
...
const buildWordCardPrompt = (term) => [
...
const normalizeTextField = (value, field) => {
...
const parseWordCardFields = (payload) => {
  let parsed;
  try {
...
const runWithTimeout = async (task, timeoutMs = DEFAULT_TIMEOUT_MS, timeoutMessage = "AI 请求超时") => {
  const controller = new AbortController();
  let timer;
...
const DEFAULT_MODEL$1 = "gemini-2.0-flash-lite-preview-02-05";
const DEFAULT_MAX_OUTPUT_TOKENS = 320;
const createGeminiProvider = (options, deps) => {
  const timeoutMs = options.timeoutMs ?? DEFAULT_TIMEOUT_MS;
  const maxOutputTokens = options.maxOutputTokens ?? DEFAULT_MAX_OUTPUT_TOKENS;
  const model = new GoogleGenerativeAI(options.apiKey).getGenerativeModel(
    {
      model: options.model ?? DEFAULT_MODEL$1,
      generationConfig: {
...
const SYSTEM_MESSAGE = "你是日语学习助手，只能输出 JSON 对象，字段包含 kana、definition_ja、scene_ja、example_ja，禁止返回与提示无关的文本或密钥。";
const createOpenAiProvider = (options) => {
...
const resolveEnvApiKey = (provider) => {
  if (provider === "openai") {
    return process.env.OPENAI_API_KEY?.trim() || void 0;
  }
  if (provider === "gemini") {
    return process.env.GEMINI_API_KEY?.trim() || process.env.GOOGLE_API_KEY?.trim() || void 0;
  }
  return void 0;
};
const normalizeProvider = (provider) => provider ?? "mock";
const normalizeApiKey = (value) => value?.trim() || void 0;
const resolveApiKey = (provider, candidate) => normalizeApiKey(candidate) ?? resolveEnvApiKey(provider);
const normalizeConfig = (current, update) => {
...
const createProviderManager = (initialSettings = {}, providerFactory = buildAiProvider) => {
...
const formatDateKey = (timestamp = Date.now()) => new Date(timestamp).toISOString().slice(0, 10);
const addActivityCounts = (activity, date, delta) => {
...
const recordAddedWord = (activity, date) => addActivityCounts(activity, date, { added_count: 1 });
const recordReviewedWord = (activity, date) => addActivityCounts(activity, date, { review_count: 1 });
const asDayNumber = (date) => Math.floor(Date.parse(`${date}T00:00:00Z`) / DAY_IN_MS);
const calculateStreak = (activity) => {
  const activeDayNumbers = Object.entries(activity.days).filter(([, value]) => value.added_count + value.review_count > 0).map(([date]) => asDayNumber(date)).filter((value) => Number.isFinite(value)).sort((a, b) => b - a);
...
const writeJsonAtomic = async (filePath, payload) => {
  ...
  await ensureDir(dir);
  const tempPath = path$1.join(dir, `${path$1.basename(filePath)}.${crypto$1.randomUUID()}.tmp`);
...
class DataStore {
  ...
  async updateWord(id, update, now = Date.now()) {
    const wordsFile = await this.readWordsFile(now);
  ...
  async deleteWord(id, now = Date.now()) {
    const wordsFile = await this.readWordsFile(now);
  ...
  async applyReview(id, grade, now = Date.now()) {
    const wordsFile = await this.readWordsFile(now);
    const index = wordsFile.words.findIndex((entry) => entry.id === id);
    if (index === -1) {
      throw new Error(`未找到指定词条：${id}`);
  ...
  async appendActivity(update, now) {
    const activity = await this.readActivityData();
...
const createMainWindow = async () => {
  const mainWindow = new BrowserWindow({
    ...
    height: 768,
    webPreferences: {
      preload: path$1.join(__dirname$1, "../preload/index.cjs"),
  ...
  if (isDev && process.env.VITE_DEV_SERVER_URL) {
    ...
    const rendererPath = path$1.join(__dirname$1, "../../dist/renderer/index.html");
