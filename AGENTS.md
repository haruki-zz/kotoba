...
Current contents are planning docs at root (`design-doc.md`, `discussion.md`, `tech-stack.md`). When code is present, keep a pnpm workspace split as: `packages/main/` (Electron main + Fastify API, SM-2 logic, SQLite access), `packages/renderer/` (React/Vite UI for Home/Today/Review/Library/Settings), `packages/shared/` (Zod schemas/types reused across processes), `data/` (local SQLite such as `data/kotoba.sqlite`, gitignored), `scripts/` for automation, and `docs/` for specs. Place page components under `packages/renderer/src/pages/`, shared UI in `.../components/`, feature logic in `.../features/<feature>/`, and tests alongside code in `__tests__` or `*.test.ts[x]`.
