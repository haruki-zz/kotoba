...
- v1 功能草案（拟定）：
  ...
  - 复习：SM-2 队列呈现；默认仅展示单词，无“熟记”按钮；用户可跳过（后端自动标记 easy 并下一条）或“展开详情”查看假名/解释/场景/例句并选择掌握度（easy/medium/hard，未选默认 medium）；支持左右箭头/快捷键双向切换并含实体“回退上一条”按钮；进度条显示今日剩余数量；跳过后无额外提示，节奏不变；当日完成后直接结束，不提示“再练 5 个 hard”；空队列文案：今日复习已完成，去巩固新增或放松一下吧。
  - 词条查看/编辑：列表+搜索；详情页支持重生成或手动覆盖各字段；编辑为悬浮二级页面、背景虚化。
  - 设置：复习队列默认 30 可自定义；例句风格=生活口语（例句 15-25 字，场景 1 句 30-40 字）；AI provider 选择；导入/导出词库（CSV 字段：词,假名,解释,场景,例句,难度,EF,interval_days,repetition,last_review_at,next_due_at）；light/dark mode 切换。
- v1 界面元素清单与页面流转草图（概念）：
  - 导航/入口：左侧或底部导航含：首页、今日学习、复习、词库、设置。
  - 首页：连续天数热力图；掌握度分布环形图；高遗忘风险 5 个 hard 词卡片（随机，可点进详情/复习）；本周新增与复习计数的折线图；今日新增/复习计数展示，无目标进度。
  ...
  - 词库：列表+搜索；点击词条打开悬浮详情页（显示全部字段）；“重生成/手动编辑”模式，保存或取消。
  - 设置：复习队列上限（默认 30，可输入）；例句风格选择（生活口语默认，例句长度 15-25 字，场景描述 1 句 30-40 字）；AI provider 选择；导入/导出 CSV（字段顺序：词,假名,解释,场景,例句,难度,EF,interval_days,repetition,last_review_at,next_due_at）；light/dark 切换。
...
- 体验与规则补充：
  ...
  - 重生成：详情页重生成时保留原版，可回滚。
- SM-2 导入/导出规则：
  - CSV 字段顺序：词,假名,解释,场景,例句,难度,EF,interval_days,repetition,last_review_at,next_due_at（ISO 8601）。
  - 缺失值处理：EF 默认 2.5；interval_days 默认 0；repetition 默认 0；时间缺失时 last_review_at 置空、next_due_at 设为导入当日。
  - 难度列：easy/medium/hard，缺失则默认 medium。
  - 时间字段采用本地时区的 ISO 8601，导出时同样输出 ISO。
  - 重名策略：导入遇到同词条时弹出交互让用户二选一（保留现有或用导入行覆盖），选择后完全覆盖另一方，不自动合并。
- 当前状态：需进一步澄清目标用户画像、核心痛点、成功指标及关键功能优先级。
- 下一步：细化核心流程、确认功能优先级与成效指标。
