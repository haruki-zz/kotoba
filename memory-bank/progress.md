...
## 2026-01-27
- 完成实施计划第 18 步：搭建左侧窄栏导航与居中单栏主区域，导航覆盖「新增单词/复习/词库/统计」四入口，顶部加入页面级标题与主行动按钮，滚动锚点定位对应区域。
- `App.tsx` 接入导航状态与主行动按钮，新增 focus token 使新增页默认聚焦输入框；新增 `LibraryHub` 汇总导入/导出与设置，并声明词库列表功能待补。
- `AddWordForm` 拆成三步骨架说明，增加实时预览卡片与手动完成按钮，保存后重新聚焦输入框。
- 样式表补充导航 item 样式，保持窄栏质感。测试由用户验证通过。

## 2026-01-28
- 完成实施计划第 19 步：复习界面改为中央单卡布局，卡片支持 200ms 翻转动画与 clamp 文本自适应；新增左右切换按钮与剩余/进度提示，评分按钮附 1/2/3 快捷键提示并支持箭头、空格与数字键操作。
