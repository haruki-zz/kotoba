...
- 渲染端基线：`packages/renderer/vite.config.ts`（Vite + React + alias）、`packages/renderer/tailwind.config.ts` 与 `postcss.config.cjs`（Tailwind 令牌绑定 CSS 变量）、`packages/renderer/index.html`（入口页）。入口 `src/main.tsx` → `src/app/App.tsx`，由 `src/app/providers.tsx` 聚合 `ThemeProvider` + toast，`src/app/routes.tsx` 通过 `HashRouter` 装配 `AppShell` 与首页/今日学习/复习/词库/设置路由。布局组件：`src/app/layout/app-shell.tsx`（侧边栏 + 顶栏 + 快捷键 Cmd/Ctrl+B/J）、`sidebar.tsx`（导航/快捷键提示）、`top-bar.tsx`。样式与令牌：`src/styles/tokens.css`（light/dark 变量、阴影/圆角/字体）、`src/styles/global.css`（Tailwind base + 背景光晕）。UI/动效：`components/ui/*`（button/badge/card/input/select/textarea）、`components/motion/presets.tsx`（淡入预设）、`components/theme/theme-toggle.tsx`、`components/a11y/skip-link.tsx`。状态/表单/API：`state/ui-store.ts`（Zustand 持久化主题/侧栏）、`theme/theme-provider.tsx`（系统偏好解析 + data-theme）、`hooks/use-api.ts` + `lib/api-client.ts`（fetch 包装 + 统一错误 toast）、`components/feedback/toaster.tsx`（sonner）、`hooks/use-zod-form.ts` + `components/form/form.tsx`（RHF + Zod 基线）。页面占位：`pages/{home,today,review,library,settings}.tsx` 展示导航结构、交互清单与设置表单示例。
- 共享 Schema 细分：
