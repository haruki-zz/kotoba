...
- 测试配置：`vitest.config.ts` 提供 alias（@kotoba/shared → packages/shared/src/index.ts，zustand 解析到 renderer 包安装位置）以在测试时直连源码并兼容 pnpm workspace。
- 渲染端基线：
  - 配置与入口：`packages/renderer/vite.config.ts`（Vite + React + alias @kotoba/shared，dev 端口 5173），`tailwind.config.ts`/`postcss.config.cjs`（Tailwind 令牌与 PostCSS），`index.html`（根容器与 script），`src/main.tsx`（StrictMode 挂载 App）。
  - 应用框架：`src/app/App.tsx`（封装 Providers + Routes）、`src/app/providers.tsx`（ThemeProvider + Toaster）、`src/app/routes.tsx`（HashRouter 路由表：主页/今日学习/复习/词库/设置）。
  - 布局与导航：`src/app/layout/app-shell.tsx`（侧栏 + 顶栏 + 主内容，快捷键 Cmd/Ctrl+B 折叠菜单、Cmd/Ctrl+J 切换主题、移动端遮罩）、`sidebar.tsx`（导航链接与快捷键提示卡片）、`top-bar.tsx`（当前路由标题/标识、主题按钮、设计准则入口）。
  - 样式与主题：`src/styles/tokens.css`（light/dark 色板、阴影、圆角、字体、背景光晕变量）、`src/styles/global.css`（Tailwind base/utility 注入、全局背景、焦点与选区样式），`src/theme/theme-provider.tsx`（system/light/dark 解析，写入 dataset/color-scheme，监听系统变更）。
  - 状态与工具：`src/state/ui-store.ts`（Zustand 持久化主题偏好与侧栏开关）、`src/lib/api-client.ts`（基于 fetch 的请求封装、统一错误类型）、`src/hooks/use-api.ts`（请求包装 + toast 错误提示）、`src/hooks/use-zod-form.ts`（Zod + RHF 组合 helper）、`src/lib/utils.ts`（类名合并等）。
  - UI/动效/可访问性：`src/components/ui/*`（Button/Badge/Card/Input/Select/Textarea 等基础组件）、`components/navigation/nav-link.tsx`（带活跃态的导航链接）、`components/layout/page-header.tsx`（页头/动作/描述）、`components/theme/theme-toggle.tsx`、`components/a11y/skip-link.tsx`、`components/motion/presets.tsx`（淡入预设）、`components/feedback/toaster.tsx`（sonner）。
  - 页面占位：`src/pages/{home,review,library,settings}.tsx` 暂存导航占位文案。
  - 今日学习流：`src/pages/today.tsx` 现已接入新增/生成/预览流程；`features/today` 模块拆分 API（words CRUD/AI 生成）、表单 schema、Zustand 局部状态（面板开合、今日列表缓存）、工具（当日过滤、摘要），组件包含悬浮表单 `TodaySheet`（RHF+Zod 校验、AI 生成、快捷键、状态提示）与 `TodayList`（今日新增列表、展开预览、刷新入口）。
- 共享 Schema 细分：
