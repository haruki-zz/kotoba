...
- 子页面
  - 復習页面：集中呈现单词卡片、复习进度条、熟悉度评分按钮（easy/medium/hard 三档），无其他干扰模块。
...
- 评分按钮：3 个按钮（hard/medium/easy 顺序），等宽排列，默认描边，悬浮填充浅橙，选中填充深橙并内阴影；文案可辅助小字标注「急需复习/需巩固/已掌握」。
...
- 卡片翻转
  ...
  - 键盘：空格或回车翻面，数字 1/2/3 对应 hard/medium/easy 打分；左右方向键切换下一词。
- 热力图
...
- 768–959px：导航栏折叠为顶部汉堡菜单；主内容改为单列垂直排列：新增区域 -> 最近列表 -> 热力图；复习页面评分按钮保持一行，必要时缩小间距与字号。
