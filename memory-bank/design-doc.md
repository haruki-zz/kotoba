...
### 4.6 学习活动 Heat Map
- 提供 `活動` 页面，并将其作为 App 默认主界面，以 heat map 展示用户最近 `40` 周的学习活跃度。
- 活跃度统计口径：
...
- Heat map 要求：
  ...
  - 显示当前词库的 `5` 级记忆等级分布：基于当前 `SM-2` 状态把单词映射到固定 `1-5` 级后，展示各等级单词数量及其占总词库单词数的百分比
- 当最近 `40` 周无活动时，仍显示空 heat map 与引导文案
