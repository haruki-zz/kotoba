...
- 简单优先：单仓、单应用，前后端统一 TypeScript；按 `packages/main`（Electron+Fastify）、`packages/renderer`（React/Vite）、`packages/shared`（Zod schema）拆分，避免巨石文件。
...
- 包管理：pnpm；格式/检查：ESLint + @typescript-eslint + Prettier；单元测试：Vitest；UI 测试：React Testing Library（核心交互）。
